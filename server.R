...
shinyServer(function(input, output, session) {
  ...
  output$netPlot <- ndtv:::renderNdtvAnimationWidget({
    ...
    # Global variables
    N <- input$PopulationSize # population size
    roundsOfPlay <- input$RoundsOfPlay # number of rounds of play with each generations
    perfectCommunication <- (input$PerfectCommunication == "Perfect") # whether agents communicate perfectly
    ...
    for (i in 1:N) {
      ...
      assign(paste("agent", i, sep = ""),
             ...
             eval(parse(
               text = paste("reliability1", i, sep = "")
             )),
             eval(parse(
               text = paste("reliability2", i, sep = "")
             )),
             eval(parse(
               text = paste("urn", i, sep = "")
             ))))
    }
